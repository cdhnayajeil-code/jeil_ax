...
--
-- 【2026-09-22 갱신 · REQ-0070 적재 후 판정】 이 파일의 뷰 정의는 아래 정본으로 대체됐다:
--   `이관/sql/61_erp_pur_list_v2.sql` — 담당자=등록자(INSRT_USER_ID) · 외주=품번 ROS 접두 ·
--   입고창고=DTL.SL_CD→창고명(DELIVERY_PLCE 는 전건 빈값) · 참조번호(REF_NO) 전건 빈값이라 제거.
--   아래 내용은 **확장 전 1차 정의**로 남겨 둔다(어떤 근거로 시작했는지 되짚기용).

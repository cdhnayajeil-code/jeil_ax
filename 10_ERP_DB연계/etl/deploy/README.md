...
## 0. 세 경로를 구분한다 — 저장소 → 전달 → 실행

| 구분 | 경로 | 성격 | 누가 채우나 |
|---|---|---|---|
| ① **정본** | 저장소 `10_ERP_DB연계/etl/` | 코드·문서를 **여기서만** 고친다 | 사람(커밋) |
| ② **전달 폴더** | 사내 OneDrive `…\jeil_ax\relay\` (경로는 `.claude/erp_relay.path`) | 서버로 건네는 창구 | **`python deploy/_sync_relay.py`** |
| ③ **실행 루트** | 서버 `E:\ai.jeil\relay\` | 러너가 **실제로 도는 자리**. `.env` 도 여기 | 관리자가 ②에서 복사 |

> **②를 건너뛰면 서버는 옛 EXE 를 계속 돈다.** 2026-09-11 에 실제로 그랬다(서버 v1.6 / 전달본 v1.7).
> 그래서 「빌드했다」와 「전달했다」를 기억이 아니라 명령 하나로 묶는다 — §0.1.

### 0.1 ERP 동기화 산출물은 이 순서로 옮긴다 (표준 절차)

```
# ① 정본을 고친다 (etl_run.py 추출 SQL · runner 기능 · 조사 SQL · 배포 문서)
# ② ETL·러너 코드가 바뀌었으면 EXE 를 다시 만든다  ← 빠뜨리기 쉬운 한 걸음
python 10_ERP_DB연계/etl/deploy/build_exe.py
# ③ 전달 폴더로 취합 (EXE + 버전 사본 + 배포 문서 + 조사 SQL + 안내문)
python 10_ERP_DB연계/etl/deploy/_sync_relay.py            # --dry-run 으로 먼저 본다
# ④ 관리자가 서버에 올린다: 러너 정지 → jeil_runner.exe 교체 → 러너 시작
# ⑤ 추출 컬럼이 늘었으면 최초 1회 백필
jeil_runner.exe etl --job <job> --full
```

**왜 ②가 필요한가** — `etl_run.py` 는 EXE 안에 통째로 박힌다(PyInstaller onefile).
추출 SQL 을 한 줄만 고쳐도 **재빌드 없이는 서버에 반영되지 않는다.**
빌드가 새 코드를 품었는지 의심되면 EXE 안의 `etl_run` 을 꺼내 상수를 확인할 수 있다
(2026-09-22 r1.4 에서 실제로 그렇게 검증했다 — `변경관리.md` 참조).

**왜 ⑤가 필요한가** — 증분 적재는 `UPDT_DT >= watermark` 로 **변경분만** 읽는다.
새로 추가한 컬럼은 기존 행에 NULL 로 남아 있어 `--full` 로 한 번 훑어야 채워진다.

### 0.2 전달 폴더에 두는 것 / 두지 않는 것

| 둔다 | 두지 않는다 |
|---|---|
| `jeil_runner.exe`(서버에 두는 **유일한** 실행 파일) | `.env` — 접속정보는 **OneDrive 를 경유시키지 않는다**(§1.1) |
| `jeil_runner.rXX-YYYYMMDD.exe` 버전 사본(되짚기용) | `pysrc/` — 통합 EXE 이전의 소스 사본. 새로 올리지 않는다 |
| `deploy/`(설치·스케줄 스크립트·런북) | `logs/`·`runner_config.json`·`runner_history.jsonl` — **서버가 쓰는 것**(반대 방향) |
| `sql/`(ERP 조사·점검 쿼리 — 읽기 전용) | 손으로 만든 임시 파일 — 다음 동기화에서 덮어써진다 |

---

## 0.3 전달 폴더에서 직접 실행하지 않는 이유

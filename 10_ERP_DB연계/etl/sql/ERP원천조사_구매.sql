...
--
-- ╔══════════════════════════════════════════════════════════════════════════╗
-- ║ 2026-09-22 **해결 완료 — §1~§6 전부 답이 나왔다. 이 파일을 다시 돌릴 필요는 없다.** ║
-- ╚══════════════════════════════════════════════════════════════════════════╝
--
--   §1 담당자   → **INSRT_USER_ID(등록자)**. AGENT 전건 빈값 · APPLICANT 전건 법인코드.
--   §2 입고처   → DELIVERY_PLCE 전건 빈값 · SL_CD 는 **저장위치**(엑셀 「입고처」와 다른 개념)
--                 → 화면 열 이름을 「입고창고」로 바꿨다.
--   §3 외주     → **품번 ROS 접두**(800건). SUBCONTRA_FLG 는 전건 'N' 이라 못 쓴다.
--   §4·§5 결재번호 → **`M_PUR_REQ.EXT1_CD`** (ERP 확장슬롯).
--       채번표 `a_tmp_auto_number` 에 PU 는 **0건**이었고(거기가 아니다),
--       `M_MRO_PUR_REQ_HDR` 에도 없었다.
--       `INTERFACE_KO174`(IF_TYPE='PR0' 8,074행)의 KEY1 이 PU 인 것은 맞지만
--       **KEY2~KEY7 이 전건 비어 있어 구매요청번호를 주지 못한다** — 그쪽만으로는 못 잇는다.
--       `ERP_IF_APPROVAL_DOCUMENT(_MAPPING)` 은 **문서양식 설정표**라 인스턴스 데이터가 아니다.
--       결국 **값으로** 찾았다 — 구매·요청 계열 문자열 컬럼 1,203개에 표본 'PU202607240005' 를
--       직접 대조해 3곳(KO174.KEY1 · M_PUR_REQ.EXT1_CD · M_PUR_GOODS_MVMT.EXT1_CD)을 찾았고,
--       **요청 단위로 붙는 것이 M_PUR_REQ.EXT1_CD** 다.
--       ⚠ 교훈: **이름으로 못 찾으면 값으로 찾는다.** ERP 확장슬롯(EXT1~3)은 이름이 뜻을 안 알려 준다.
--       같은 슬롯의 EXT2_CD=건명 · EXT3_CD=도번/품명 도 함께 쓴다.
--       실측(2026): 요청 5,551라인 · 결재번호 5,292(95.3%) · 고유 1,133 ↔ 엑셀 고유 1,060.
--   §6 단가     → PO_PRC 4,618행 전건 채워짐.
--
-- 적용: `이관/sql/60`(발주 11컬럼) · `65`(요청 3컬럼+전용 upsert) · `66`(뷰 pu_no 실체화).
-- 아래 쿼리는 **되짚기용 기록**으로 남긴다.
--

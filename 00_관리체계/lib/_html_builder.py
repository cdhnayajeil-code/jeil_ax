# -*- coding: utf-8 -*-
"""
JEIL AX 공통 문서 빌더 — .md → 공유 스타일 HTML.
'실제구축준비 자료/이관' · '08/03_실구축기획' 빌드 스크립트의 공통 로직을 한곳에 모았다.
각 폴더의 _build_html.py 는 DOCS 목록과 라벨만 정의하고 build_docs() 를 호출한다.

사용(각 폴더 _build_html.py):
    import sys, os
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', '00_관리체계', 'lib'))
    from _html_builder import build_docs
    build_docs(HERE, DOCS, doc_label='...', title_mid='...', footer_left='...',
               hub_label='...', hub_dashboard_label='...')
...
def build_docs(here, docs, *, doc_label, title_mid, footer_left,
               hub_label, hub_dashboard_label, manager='최동혁'):
    """here 폴더의 docs(각 (slug,label,sub))를 동일 스타일 HTML 로 빌드한다."""
...
<title>{label} — {title_mid} ({sub})</title>
<style>{CSS}</style>
</head>
<body>
<div class="page">
...
{body}
  ...
  <div class="footer"><span>{footer_left}</span><span>{label} · 관리: {manager}</span></div>
</div>
</body>
</html>"""
